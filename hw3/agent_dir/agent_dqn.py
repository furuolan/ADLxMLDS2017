...
INITIAL_EPSILON = 1 #1.0 # Initial value of epsilon in epsilon-greedy
...
NUM_REPLAY_MEMORY = 80000  # Number of replay memory the agent uses for training
BATCH_SIZE = 32  # Mini batch size
TARGET_UPDATE_INTERVAL = 4000  # The frequency with which the target network is updated
...
class Agent_DQN(Agent):
    ...
    def train(self):
        ...
        for _ in range(NUM_EPISODES):
            ...
            state = observation#self.get_initial_state(observation, last_observation)
            while not terminal:
                last_observation = observation
                ...
                state = self.run(last_observation, action, reward, terminal, observation)
    ...
    def make_action(self, observation, test=True):
        ...
        return self.get_action_at_test(observation)
    ...
    def change_shape(self, state):
        ...
        newstate = np.zeros((FRAME_HEIGHT,FRAME_WIDTH,STATE_LENGTH))
