...
#'Consolidation', 'Edema' , 'Emphysema','Fibrosis', 'Hernia' , 'Pleural_Thickening'
...
train_name = pd.read_csv(datadir+"train.txt", header= None)
...
# val_box=valid_name.join(BBox_List_2017.set_index("Image Index"),how="inner",on=0)


# # In[22]:


# test_box=test_name.join(BBox_List_2017.set_index("Image Index"),how="inner",on=0)
...
# from GlobalLSE import GlobalLSEPooling2D
# model = DenseNetImageNet121(input_shape=(224,224,1), include_top=False, weights=None)
...
# model = InceptionResNetV2(include_top=False,weights=None,input_shape=(256,256,1))
# x = model.output
# x = GlobalAveragePooling2D(name='avg_pool')(x)
# predictions = Dense(classes, activation='sigmoid', name='predictions')(x)
...
# model = VGG16(include_top=False,weights=None,input_shape=(256,256,1))
# x = model.output
...
batch_size = 32
f_name = train_name[0].values
f = label_codelist.loc[f_name].dropna()
x_name = f.index.values
train_y = f.values
#train_generator = xraySequence(x_name,train_y,batch_size)
...
train_x = np.empty((len(x_name),224,224,1),dtype='float16')

for i,file_name in enumerate(x_name):
    img = cv2.imread(image_dir+file_name,cv2.IMREAD_GRAYSCALE)
    img = cv2.resize(img,(224,224),interpolation=cv2.INTER_LANCZOS4)
    train_x[i] = img.reshape((224,224,1))


# In[41]:


val_name = valid_name[0].values
v = label_codelist.loc[val_name].dropna()
val_x_name = v.index.values
val_y = v.values
#val_generator = xraySequence(val_x_name,val_y,batch_size)


# In[42]:


val_x = np.empty((len(val_x_name),224,224,1),dtype='float16')

for i,file_name in enumerate(val_x_name):
    img = cv2.imread(image_dir+file_name,cv2.IMREAD_GRAYSCALE)
    img = cv2.resize(img,(224,224),interpolation=cv2.INTER_LANCZOS4)
    val_x[i] = img.reshape((224,224,1))


# In[43]:


test_name = test_name[0].values
t = label_codelist.loc[test_name].dropna()
test_x_name = t.index.values
test_y = t.values
...
test_x = np.empty((len(test_x_name),224,224,1),dtype='float16')

for i,file_name in enumerate(test_x_name):
    img = cv2.imread(image_dir+file_name,cv2.IMREAD_GRAYSCALE)
    img = cv2.resize(img,(224,224),interpolation=cv2.INTER_LANCZOS4)
    test_x[i] = img.reshape((224,224,1))
...
checkpoint = ModelCheckpoint(filepath='best_augement_14_honhon_weight.hdf5',
                             verbose=1,
                             save_best_only=True,
