...
tags_clean=tags[1].apply(lambda x: re.sub(":\d+","", x).split('\t'))
...
tags[1]=tags_clean.apply(finding)
new_tags=tags.dropna()

all_tags = set()
for i in new_tags[1].values:
    for j in i:
        all_tags.add(j)

from sklearn.preprocessing import LabelBinarizer
lb = LabelBinarizer()
lb.fit(list(all_tags))
...
def bagofword(x):
    vector = np.zeros(len(lb.classes_))
    for tag in x:
        vector += lb.transform([tag]).reshape(len(lb.classes_))
    return vector
	
tags_vector=new_tags[1].apply(bagofword)

vectors = np.zeros((tags_vector.shape[0],classnum))
for i,vector in enumerate(tags_vector.tolist()):
    vectors[i,:] = vector
...
resume_model = False
model_path = "Data/Models/latest_model_temp.ckpt"
...
caption=["red hair,green eyes","blue hair,red eyes","black hair,blue eyes"]
...
for i in range(epochs):
    ...
    while batch_no*batch_size < loaded_data['data_length']:
        ...
        if (batch_no % save_every)== 0:
            ...
            save_path = saver.save(sess, "Data/Models/latest_model_temp.ckpt")
