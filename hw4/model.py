...
class GAN:
    ...
    def build_model(self):
        ...
        t_wrong_caption = tf.placeholder('float32', [self.options['batch_size'], self.options['caption_vector_length']], name = 'wrong_caption_input')
        ...
        disc_mismatch_image, disc_mismatch_image_logits   = self.discriminator(t_real_image, t_wrong_caption, reuse = True)
...
#         d_loss2 = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=disc_mismatch_image_logits, labels=tf.zeros_like(disc_mismatch_image)))
#         d_loss3 = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=disc_fake_image_logits, labels=tf.zeros_like(disc_fake_image)))
        g_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=disc_fake_image_logits, labels=tf.ones_like(disc_fake_image_logits)))

        d_loss1 = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=disc_real_image_logits, labels=tf.ones_like(disc_real_image)))
        d_loss2 = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=disc_mismatch_image_logits, labels=tf.zeros_like(disc_mismatch_image)))
        d_loss3 = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=disc_fake_image_logits, labels=tf.zeros_like(disc_fake_image)))
        d_loss4 = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=disc_wrong_image_logits, labels=tf.zeros_like(disc_wrong_image)))
        ...
        checks = {
            ...
            'disc_mismatch_image_logits' : disc_mismatch_image_logits,
    ...
    def build_discriminator(self):
        ...
        d_loss4 = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=disc_wrong_image_logits, labels=tf.zeros_like(disc_wrong_image)))
        d_loss = d_loss1 + (d_loss2 + d_loss4)/2
    ...
    def generator(self, t_z, t_text_embedding):
        with tf.variable_scope("generator") as scope:
            ...
            #noise=tf.random_normal(mean=0,stddev=0.2,shape=t_text_embedding.get_shape(),dtype=tf.float32)
            reduced_text_embedding = ops.lrelu( ops.linear(t_text_embedding, self.options['t_dim'], 'g_embedding') )
            z_concat = tf.concat([t_z, reduced_text_embedding], 1)
            z_ = ops.linear(z_concat, self.options['gf_dim']*8*s16*s16, 'g_h0_lin')
            h0 = tf.reshape(z_, [-1, s16, s16, self.options['gf_dim'] * 8])
